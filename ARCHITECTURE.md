...
- **Re-ID Fusion**: Trích xuất vector 512 chiều bằng OSNet/TransReID để nhận diện lại đối tượng cross-camera.
- **Xử lý quỹ đạo (Trajectory)**:
  - Chuẩn hóa tọa độ theo tỉ lệ `[0.0..1.0]` độc lập với độ phân giải video.
  - Định vị theo trọng tâm đối tượng, làm mịn chống rung box bằng bộ lọc Gaussian 5-point moving average và hiển thị đường cong mượt trên Canvas.
...
Lưu trữ thông tin quản trị và xác thực trong `admin.db`:
- **`api_keys` Table**:
...
1. **Xác Thực Admin**:
   ...
   - Token mã hóa HMAC-SHA256 kèm thời hạn hết hạn 24 giờ.
